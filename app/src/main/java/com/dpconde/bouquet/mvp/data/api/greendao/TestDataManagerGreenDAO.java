package com.dpconde.bouquet.mvp.data.api.greendao;

import com.dpconde.bouquet.mvp.Constants;
import com.dpconde.bouquet.mvp.data.model.Checklist;
import com.dpconde.bouquet.mvp.data.model.TaskDao;
import com.dpconde.bouquet.mvp.data.model.TaskType;
import com.dpconde.bouquet.mvp.data.model.TaskTypeDao;
    ...
    public Checklist saveChecklist(Checklist checklist){
        ...
        for(Task task: checklist.getTasks()){
            ...
            if(task.getTaskType().getId() == Constants.TASK_TYPE_CONTAINER_ID){
                long id = daoSession.getContainerTaskDataDao().insertOrReplace(task.getContainerTaskData());
                task.setContainerTaskDataId(id);

            }else if(task.getTaskType().getId() == Constants.TASK_TYPE_GENERAL_ID){
                long id = daoSession.getGeneralTaskDataDao().insertOrReplace(task.getGeneralTaskData());
                task.setGeneralTaskDataId(id);

            }else if(task.getTaskType().getId() == Constants.TASK_TYPE_CHECKLIST_ID){
                long id = daoSession.getChecklistTaskDataDao().insertOrReplace(task.getChecklistTaskData());
                task.setChecklistTaskDataId(id);

            }else if(task.getTaskType().getId() == Constants.TASK_TYPE_MISUSE_ID){
                long id = daoSession.getMisuseTaskDataDao().insertOrReplace(task.getMisuseTaskData());

                //Insert drivers
                for(MisuseDriver driver: task.getMisuseTaskData().getDriverList()){
                    driver.setMisuseTestId(id);
                    daoSession.getMisuseDriverDao().insertOrReplace(driver);
                }

                task.setMisuseTaskDataId(id);
            }
